# Notebook Lab Spec Roadmap

## Infrastructure

- [T00.4 — Spec-Driven Execution System](infrastructure/T00.4-spec-driven-workflow.md) — `repository_finalized`

## Product

### task-foundation — `feature/task-foundation`
- [T01 — Domain and repository](task-foundation/T01-domain-repository.md) — `repository_finalized`
- [T02 — Local persistence](task-foundation/T02-local-persistence.md) — `repository_finalized`

### app-shell — `feature/app-shell`
- [T03 — App shell](app-shell/T03-app-shell.md) — `repository_finalized`
- [T04 — Shared UI primitives](app-shell/T04-ui-primitives.md) — `repository_finalized`

### today-tasks — `feature/today-tasks`
- [T05 — Today task list](today-tasks/T05-today-list.md) — `repository_finalized`
- [T06 — Create task](today-tasks/T06-create-task.md) — `repository_finalized`
- [T07 — Complete/reopen](today-tasks/T07-complete-reopen.md) — `repository_finalized`
- [T08 — Edit/delete](today-tasks/T08-edit-delete.md) — `repository_finalized`

### daily-summary — `feature/daily-summary`
- [T09 — Daily summary](daily-summary/T09-daily-summary.md) — `repository_finalized`

### monthly-summary — `feature/monthly-summary`
- [T10 — Monthly aggregation](monthly-summary/T10-monthly-aggregation.md) — `planned`
- [T11 — Monthly summary UI](monthly-summary/T11-monthly-summary-ui.md) — `planned`

### ux-quality — `feature/ux-quality`
- [T12 — Responsive/accessibility](ux-quality/T12-responsive-accessibility.md) — `planned`
- [T13 — Motion/performance](ux-quality/T13-motion-performance.md) — `planned`

### final — `chore/final-audit`
- [T14 — Final audit](final/T14-final-audit.md) — `planned`
